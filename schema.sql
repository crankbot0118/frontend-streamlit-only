...
CREATE TABLE environments (
    ...
    locked          BOOLEAN         NOT NULL DEFAULT FALSE              -- TRUE while this environment is the target of an active clone run; prevents concurrent writes to the same target
);
...
--  Fires after any INSERT or status UPDATE on clone_function_run_status.
--  Rolls up step states into the parent clone_run_status row:
--
--    Priority: ABORTED > FAILED > RUNNING > COMPLETED > PENDING
--
--  Also stamps start_date on clone_run_status the first time any
...
FOR EACH ROW
EXECUTE FUNCTION sync_clone_run_status();
...
DECLARE
    ...
    v_fid         INTEGER;
BEGIN
    -- Guard: reject if target environment is already locked
    IF EXISTS (
...
--  Unlocks the target environment when a run reaches a terminal state
--  (COMPLETED, FAILED, ABORTED). Call from your agent after the
--  run concludes, or wire into a trigger if preferred.
-- ============================================================
