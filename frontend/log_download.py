...
@st.cache_data(show_spinner=False, ttl=300)
def cached_step_log_text(clone_run_id: int, clone_function_run_id: int) -> tuple[str, str]:
    """Return step log body as text plus the suggested filename."""
    data, name = fetch_step_log(clone_run_id, clone_function_run_id)
    return data.decode("utf-8", errors="replace"), name
