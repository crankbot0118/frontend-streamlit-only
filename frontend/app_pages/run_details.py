...
def _render_run_header_actions(*, run_id: int, run_status: str) -> None:
    """Completed runs get a direct View Log link; others get an arrow action menu."""
    is_completed = (run_status or "").upper() == "COMPLETED"
    open_key = f"run_actions_open_{run_id}"
    ...
    with st.container(key="detail-meta-actions"):
        ...
        with more_col:
            if is_open:
                st.html('<span class="ca-step-more">Actions</span>')
        with arrow_col:
            st.button(
                "",
                key=f"run_more_{run_id}",
                icon=":material/arrow_right:",
                type="tertiary",
                on_click=_toggle_run_actions,
                args=(open_key,),
            )
        if is_open:
            with st.container(key="run_actions_links"):
                ...
                st.button(
                    ...
                    disabled=True,
                    help="Abort is not available yet.",
                )
