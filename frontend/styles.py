...
<style>
  ...
  .ca-kpi-table tbody tr:last-child td {{
      border-bottom: 0;
  }}
  .ca-kpi-table .jid {{
      font-weight: 700;
...
def kpi_card_html(
    ...
    *,
    ...
    delta_class = delta_tone if delta_tone in {"up", "down", "neutral"} else "neutral"
    delta_html = (
        f'<span class="ca-kpi-pill {delta_class}">{html.escape(delta)}</span>'
        if delta
        else ""
    )
    note_html = f'<span class="ca-kpi-note">{html.escape(note)}</span>' if note else ""
    return (
        f'<div class="ca-kpi-card">'
        f'<div class="ca-kpi-top"><span class="ca-kpi-ic">{html.escape(icon)}</span>'
        f"{html.escape(title)}</div>"
        f'<div class="ca-kpi-val">{html.escape(value)}{unit_html}</div>'
        f'<div class="ca-kpi-foot">{delta_html}{note_html}</div>'
        f"</div>"
    )


def kpi_cards_row_html(cards: list[str]) -> str:
    return f'<div class="ca-kpis">{"".join(cards)}</div>'


def outcome_donut_html(outcomes: dict[str, int], success_rate: float | None) -> str:
    """Donut + legend for success / failed / aborted."""
    success = outcomes.get("success", 0)
    failed = outcomes.get("failed", 0)
    aborted = outcomes.get("aborted", 0)
    total = success + failed + aborted
    center = f"{success_rate:.1f}%" if success_rate is not None else "—"
    if total == 0:
        segments = ""
        rows = (
            '<div class="ca-kpi-drow"><span class="ca-kpi-sw" style="background:#e3e6e8"></span>'
            '<span class="nm">No data</span><span class="num">0</span><span class="pct">—</span></div>'
        )
    else:
        pct_success = 100.0 * success / total
        pct_failed = 100.0 * failed / total
        pct_aborted = 100.0 * aborted / total
        circumference = 100.0
        dash_success = pct_success
        offset_failed = -pct_success
        dash_failed = pct_failed
        offset_aborted = -(pct_success + pct_failed)
        dash_aborted = pct_aborted
        segments = (
            f'<circle cx="18" cy="18" r="15.9" fill="none" stroke="#e3e6e8" stroke-width="3.6"></circle>'
            f'<circle cx="18" cy="18" r="15.9" fill="none" stroke="{BRAND_ORANGE}" stroke-width="3.6" '
            f'stroke-dasharray="{dash_success:.1f} {circumference - dash_success:.1f}" '
            f'stroke-dashoffset="25" stroke-linecap="round" transform="rotate(-90 18 18)"></circle>'
            f'<circle cx="18" cy="18" r="15.9" fill="none" stroke="{BRAND_RED}" stroke-width="3.6" '
            f'stroke-dasharray="{dash_failed:.1f} {circumference - dash_failed:.1f}" '
            f'stroke-dashoffset="{25 + offset_failed:.1f}" stroke-linecap="round" transform="rotate(-90 18 18)"></circle>'
            f'<circle cx="18" cy="18" r="15.9" fill="none" stroke="#8a9097" stroke-width="3.6" '
            f'stroke-dasharray="{dash_aborted:.1f} {circumference - dash_aborted:.1f}" '
            f'stroke-dashoffset="{25 + offset_aborted:.1f}" stroke-linecap="round" transform="rotate(-90 18 18)"></circle>'
        )

        def _row(label: str, count: int, pct: float, color: str) -> str:
            pct_text = f"{pct:.1f}%" if total else "—"
            return (
                f'<div class="ca-kpi-drow"><span class="ca-kpi-sw" style="background:{color}"></span>'
                f'<span class="nm">{html.escape(label)}</span>'
                f'<span class="num">{count}</span><span class="pct">{pct_text}</span></div>'
            )

        rows = (
            _row("Success", success, pct_success, BRAND_ORANGE)
            + _row("Failed", failed, pct_failed, BRAND_RED)
            + _row("Aborted", aborted, pct_aborted, "#8a9097")
        )

    return (
        f'<div class="ca-kpi-donut-wrap">'
        f'<div class="donut" style="position:relative;width:172px;height:172px;">'
        f'<svg viewBox="0 0 36 36" width="172" height="172">{segments}</svg>'
        f'<div style="position:absolute;inset:0;display:flex;flex-direction:column;'
        f'align-items:center;justify-content:center;">'
        f'<div style="font-weight:800;font-size:30px;letter-spacing:-1px;color:{BRAND_INK};">'
        f"{html.escape(center)}</div>"
        f'<div style="font-size:11px;color:#6b7177;font-weight:600;margin-top:1px;">success rate</div>'
        f"</div></div>"
        f'<div class="ca-kpi-dlegend">{rows}</div>'
        f"</div>"
    )


def latest_runs_table_html(runs: list[dict]) -> str:
    """Compact latest-runs table for KPI dashboard."""
    if not runs:
        return '<p class="ca-kpi-note" style="margin:0;padding:8px 10px;">No runs this week.</p>'
    rows_html = []
    for run in runs:
        rid = html.escape(str(run.get("clone_run_id", "—")))
        client = html.escape(str(run.get("client_name") or "—"))
        flow = (
            f'{html.escape(str(run.get("source_name") or "—"))}'
            f' <span style="color:#8a9097;">→</span> '
            f'{html.escape(str(run.get("target_name") or "—"))}'
        )
        duration = html.escape(fmt_duration(run.get("start_date"), run.get("last_update")))
        status = status_badge_html(run.get("status", ""))
        when = relative_update_html(run.get("last_update"))
        rows_html.append(
            f"<tr>"
            f'<td class="jid">#{rid}</td>'
            f"<td>{client}</td>"
            f'<td class="flow">{flow}</td>'
            f'<td class="tmuted">{duration}</td>'
            f"<td>{status}</td>"
            f'<td class="tmuted">{when}</td>'
            f"</tr>"
        )
    return (
        '<table class="ca-kpi-table">'
        "<thead><tr>"
        "<th>Run</th><th>Client</th><th>Flow</th><th>Duration</th><th>Status</th><th>Updated</th>"
        "</tr></thead>"
...
NAV: list[dict] = [
    ...
    {"kind": "item", "title": "KPI Metrics", "icon": "analytics",
     "key": "nav_kpi", "module": "app_pages/kpi_metrics.py"},
